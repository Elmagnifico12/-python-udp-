...
filePath = "/Users/elmagnifico/Desktop/python/com/Task2_P2P/Peer/Peer1Data/"  # 文件存储路径
BUFFSIZE = 2048  # 文件传输缓存
PEER_IP = "127.0.0.1"  # 节点IP地址
PEER_PORT = 8001  # 节点端口
PEER_DOWNLOAD_IP = "127.0.0.1"  # 用于其他节点下载的IP地址
PEER_DOWNLOAD_PORT = 8002  # 用于其他节点下载的端口
...
class Client:
    ...
    def send_segment(self, SYN, ACK, SEQ, FUNC, serverName, port, data=b""):
        ...
        print("send segment to %s:%d --- SYN: %d ACK: %d SEQ: %d FUNC: %d rwnd: %d" % (
        serverName, port, SYN, ACK, SEQ, FUNC, self.rwnd))
    ...
    def receive_segment(self):
        ...
        addr[0], addr[1], SYN, ACK, SEQ, FUNC, rtrwnd))
...
class Interface:
    ...
    def send_segment(self, SYN, ACK, SEQ, FUNC, data=b""):
        ...
        print("send segment to %s:%d --- SYN: %d ACK: %d SEQ: %d FUNC: %d rwnd: %d" % (
        self.addr[0], self.addr[1], SYN, ACK, SEQ, FUNC, self.rwnd))
...
class Server:
    ...
    def delete_interface(self, addr):
        print("delete the interface: "+str(addr))
